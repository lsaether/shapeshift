...
fn main() {
    ...
    let matches = App::new(APP_NAME)
        ...
        .subcommand(
            SubCommand::with_name("get-coins")
    ...
    if let Some(ref matches) = matches.subcommand_matches("shift") {
        ...
        if !matches.is_present("withdrawal_address") {
            ...
            w_addr = String::from(matches.value_of("withdrawal_address").unwrap());
        }
        ...
        if !matches.is_present("pair") {
            ...
            pair = String::from(matches.value_of("pair").unwrap());
        }
        ...
        if !matches.is_present("return") {
            ...
            if r_addr.is_empty() {
                if fixed {
                    let response =
                        shapeshift::transaction::FxTx::shift(&amount, &w_addr, &pair, "");
                    println!("{}", &response);
                } else {
                ...
                if fixed {
                    let response =
                        shapeshift::transaction::FxTx::shift(&amount, &w_addr, &pair, &r_addr);
                    println!("{}", response);
                } else {
