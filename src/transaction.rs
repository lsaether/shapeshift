...
Type `shapeshift-rs status {}` to check status of transaction.",
            self.deposit_type, self.deposit, self.withdrawal_type, self.withdrawal, self.deposit
        )
    }
}
...
impl Tx {
    pub fn shift(waddr: &str, pair: &str, raddr: &str) -> Tx {
        ...
        let mut res = client.post(&uri).json(&post).send().unwrap();
...
impl FxTx {
    pub fn shift(amt: &str, waddr: &str, pair: &str, raddr: &str) -> FxTx {
        use std::collections::HashMap;
        use std::io::Read;
...
pub fn get_tx_status(address: &str) -> String {
    ...
    if content.contains("no_deposits") || content.contains("received") {
        ...
        let finish = format!(
            "\nGot status {} on transaction to address {}",
            s.status, s.address
        );
        return finish;
        ...
        let finish = String::from(
            "Something went wrong... Send an email to the author at Lsaether@protonmail.com",
        );
        return finish;
    }
}
